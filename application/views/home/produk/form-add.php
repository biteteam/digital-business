    ...
    <div class="row px-xl-5" style="display: flex; justify-content: center;">
        <div class="col-lg-7 mb-5">
            <div class="contact-form">
                <div id="success"></div>
                <?= form_open_multipart('produk/add') ?>
                <input type="hidden" name="id-toko" value="<?= $idToko ?>" />
                <div class="control-group mb-3">
                    <select class="form-control" name="id-kategori">
                        <?php foreach ($kategori as $kt) : ?>
                            <option value="<?= $kt->idKat ?>"><?= $kt->namaKat ?></option>
                        <?php endforeach ?>
                    </select>
                    <span class="text-danger"><small><?= form_error('kategori') ?></small></span>
                </div>
                <div class="control-group mb-3">
                    <input type="text" class="form-control" id="name" name="nama" value="<?= set_value('nama') ?>" placeholder="Nama Produk" />
                ...
                <div class="control-group mb-3">
                    <input type="number" type="number" min="0" step="any" class="form-control" id="berat" name="berat" value="<?= set_value('berat') ?>" placeholder="Berat Produk" />
                    <span class="text-danger"><small><?= form_error('berat') ?></small></span>
                </div>
                <div class="control-group mb-3">
                    <textarea class="form-control" rows="3" id="deskripsiProduk" name="deskripsi" placeholder="Deskripsi"><?= set_value('deskripsi') ?></textarea>
